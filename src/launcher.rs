...
pub struct Launcher {
	ipc_stream : ::stream::IpcStream,
	symm_nonce : sodiumoxide::crypto::secretbox::xsalsa20poly1305::Nonce,
	symm_key : sodiumoxide::crypto::secretbox::xsalsa20poly1305::Key, 
}

impl Launcher {
	pub fn new(main_arg : &str) -> Launcher {
		// --------------------------------------------------------------------------------
		//                               Authentication
		// --------------------------------------------------------------------------------
	...
	pub fn mkdir(&mut self, create_dir_data : CreateDirData) {
		...
		self.ipc_stream.write(create_dir_json_str_encrypted);

		// Wait for Launcher response.
		let payload = self.ipc_stream.read_payload();
		// Decrypt using the Secret Key.
		let payload_decrypted = sodiumoxide::crypto::secretbox::open(&payload,
																	 &self.symm_nonce,
																	 &self.symm_key).unwrap_or_else(|a| panic!("{:?}", a));
